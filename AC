...
CMDs[#CMDs + 1] = {NAME = 'placeid', DESC = 'Copies the games PlaceId to your clipboard'}
...
CMDs[#CMDs + 1] = {NAME = 'noadds', DESC = 'Prevents the game from showing you purchase/premium prompts'}
...
CMDs[#CMDs + 1] = {NAME = 'gotowp / wp [name]', DESC = 'Teleports player to a waypoint'}
...
CMDs[#CMDs + 1] = {NAME = 'timetp / ptp [plr] [seconds]', DESC = 'Teleports you to a player for a specified ammount of time'}
...
CMDs[#CMDs + 1] = {NAME = 'mute [plr]', DESC = 'Mutes someones boombox'}
CMDs[#CMDs + 1] = {NAME = 'unmute [plr]', DESC = 'Unmutes someones boombox'}
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if toClipboard then
		toClipboard('https://discord.gg/NM8bkwHrfq')
		notify('Discord Invite', 'Copied to clipboard!\ndiscord.gg/NM8bkwHrfq')
	else
		notify('Discord Invite', 'discord.gg/NM8bkwHrfq')
	end
	if httprequest then
		httprequest({
...
addcmd('placeid',{},function(args, speaker)
	local jobId = ''..game.PlaceId..''
	toClipboard(jobId)
end)
...
addcmd('gotowp',{'wp','lpos','loadposition','loadpos'},function(args, speaker)
	local WPName = tostring(getstring(1))
...
addcmd('timetp',{'ptp'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		if Players[v].Character ~= nil then
...
addcmd('mute',{},function(args, speaker)
...
addcmd('unmute',{},function(args, speaker)
	if not notifiedRespectFiltering and SoundService.RespectFilteringEnabled then notifiedRespectFiltering = true notify('RespectFilteringEnabled','RespectFilteringEnabled is set to true (the command will still work but may only be clientsided)') end
	local players = getPlayer(args[1], speaker)
	if players ~= nil then
		for i,v in pairs(players) do
